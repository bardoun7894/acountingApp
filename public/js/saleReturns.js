...
export default class SaleReturns {
    constructor() {
    ...
    initializeEvents() {
      ...
      onQtyChange(){
        ...
        $(document).on("change", "#returnSaleQty",function () {
            // var errorMessage = document.getElementById('error-message');
                ...
                var sale_quantity = $("#sale_quantity" + id).text();
                ...
                getSelectorBasedInOther(
                    { sale_qty: quantity, idReturnDetails: idReturnDetails, invoice_number: invoiceNumber },
                        "return_sales_on_qty_change"
                ).then((data) => {
                    console.log(data);
                    console.log("data");
                     
                     $("#sub_total_sale_returned").text(data);
                    });
            if( parseInt(sale_quantity)< quantity){
                    // make border if the quantity is greater
                    $(this).css("border", "1px solid red");
                    errorMessage.textContent = 'Return quantity should not be greater than Sale quantity';
                }else{
                    $(this).css("border", "1px solid green");
                    errorMessage.textContent = '';
                }
                $("#total_amount_sales_returned" + id).text((quantity * unitPrice));
    ...
    findInvoiceBtn(){
        $('#findInvoiceBtn').on('click',function() {
            ...
            getSelectorBasedInOther({'invoice_number': invoiceNumber }, "find-invoice-sale"
            ).then((data ) => {
                $('#appendSaleReturnLevel').html(data);
            });
