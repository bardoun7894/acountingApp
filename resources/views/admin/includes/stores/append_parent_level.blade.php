...
<div class="row justify-content-md-center form-group">
    <div class="col-md-6">
        ...
        <option id="mainCategory" value="0" @if( isset($categoryData['parent_id']) && $categoryData->parent_id==0) selected="" @endif  >{{__("messages.main_category")}}</option>
{{-- <input id="catid" value="{{$categoryData->id}}" hidden>--}}
            @if(!empty($getCategories))
                @foreach($getCategories as $category)
        {{--             this for category page--}}
            <option     @if(isset($categoryData['parent_id'])&& $category->id == $categoryData->parent_id)
