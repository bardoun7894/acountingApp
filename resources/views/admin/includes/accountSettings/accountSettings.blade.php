                    ...
                    <div class="heading-elements">
                        <ul class="list-inline mb-0">
                            <li><a class="btn btn-sm btn-blue box-shadow-2 round btn-min-width pull-right"
                                    href="{{ url('accountSettings/create') }}"
                                    target="_self">{{ __('messages.add_account_setting') }}</a></li>
                        </ul>
                    </div>
                </div>
                            ...
                            <tbody id="accountSettings-dynamicRow">
                                @foreach ($accountSettings as $key => $accountSetting)
                                    <tr>
                                        ...
                                        <td class="text-truncate">
                                            {{ $accountSetting->accountSubControl->$account_sub_control_name }}</td>
                                        <td class="text-truncate">
                                            {{ $accountSetting->accountActivity->$account_activity_name }}</td>
                                        <td class="text-truncate"> <a
                                                href="{{ url('accountSettings/' . $accountSetting->id . '/edit') }}"><i
                                                    class="la la-edit" style="color: green;font-size: 25px"></i></a>
                                        </td>
                                        <td>
                                            <a class="confirmDelete" record="AccountSetting"
                                                recordId="{{ $accountSetting->id }}"> <i class="la la-trash"
